...
from LossFunction import DWLoss
...
def run_model(SEED, DATASET, MODEL, K_Fold):
    ...
    assert DATASET in ["DrugBank", "Enzyme", "GPCRs""]
    ...
    for i_fold in range(K_Fold):
        ...
        train_dataset = CustomDataSet(train_dataset, DATASET)
        valid_dataset = CustomDataSet(valid_dataset, DATASET)
        test_dataset = CustomDataSet(test_data_list, DATASET)
        ...
        LOSS = DWLoss(weight_loss=weight_loss, DEVICE=DEVICE)
        ...
        for epoch in range(1, hp.Epoch + 1):
            ...
            for train_i, train_data in train_pbar:
                train_compounds, train_proteins, train_compounds_bert, train_proteins_bert, train_labels, train_morgan_fp, train_avalon_fp, train_maccs_fp, train_protein_features = train_data
                train_compounds = train_compounds.to(DEVICE)
                train_proteins = train_proteins.to(DEVICE)
                train_compounds_bert = train_compounds_bert.to(DEVICE)
                train_proteins_bert = train_proteins_bert.to(DEVICE)
                train_labels = train_labels.to(DEVICE)
                train_morgan_fp = train_morgan_fp.to(DEVICE)
                train_avalon_fp = train_avalon_fp.to(DEVICE)
                train_maccs_fp = train_maccs_fp.to(DEVICE)
                train_protein_features = train_protein_features.to(DEVICE)
                optimizer.zero_grad()

                outputs = model(train_compounds, train_proteins, train_morgan_fp, train_avalon_fp, train_maccs_fp,
                                train_protein_features, train_compounds_bert, train_proteins_bert)


                train_loss = LOSS(outputs, train_labels)  # 使用完整的outputs
            ...
            with torch.no_grad():
                for valid_i, valid_data in valid_pbar:
                    valid_compounds, valid_proteins, valid_compounds_bert, valid_proteins_bert, valid_labels, valid_morgan_fp, valid_avalon_fp, valid_maccs_fp, valid_protein_features = valid_data
                    valid_compounds = valid_compounds.to(DEVICE)
                    valid_proteins = valid_proteins.to(DEVICE)
                    valid_compounds_bert = valid_compounds_bert.to(DEVICE)
                    valid_proteins_bert = valid_proteins_bert.to(DEVICE)
                    valid_labels = valid_labels.to(DEVICE)
                    valid_morgan_fp = valid_morgan_fp.to(DEVICE)
                    valid_avalon_fp = valid_avalon_fp.to(DEVICE)
                    valid_maccs_fp = valid_maccs_fp.to(DEVICE)
                    valid_protein_features = valid_protein_features.to(DEVICE)

                    outputs = model(valid_compounds, valid_proteins, valid_morgan_fp, valid_avalon_fp,
                                    valid_maccs_fp, valid_protein_features, valid_compounds_bert,
                                    valid_proteins_bert)
                    valid_scores = outputs[0]  # 只取final_pred


                    valid_loss = LOSS(outputs, valid_labels)  # 使用完整的outputs
            ...
            # 保存最佳模型
            if Accuracy_dev > best_valid_accuracy:
                best_valid_accuracy = Accuracy_dev
                best_epoch = epoch
                torch.save(model.state_dict(), save_path + '/best_model_fusion.pth')

        '''加载最佳模型'''
        model.load_state_dict(torch.load(save_path + '/best_model_fusion.pth'))
